---
slug: prime-scheduled-multiresolution-contraction
status: draft
created: '2026-03-20'
updated: '2026-03-20'
version: 0.1.0
...
- multiplicity
traceability:
  path: 03-case-studies/Prime-Scheduled Multiresolution Contraction.md
  last_synced: '2026-03-20T17:17:20.753816Z'
---

Prime-Scheduled Multiresolution Contraction v4.1
(Final) — Formal Specification, Experimental
...
$$ U_j = M_j \odot \Big( -\eta\,G - (1-a_j)X + \text{(optional noise)} \Big) $$
...
Finally, update in parameter space:
                                                          ...
                                                          1
$$ \Delta x_t = \mathrm{irfft}(U),\qquad x_{t+1}=x_t+\Delta x_t $$


Key invariants frozen in v4.1


      • Local RNG only inside schedule generation (no global RNG pollution).
      • DC always active (prevents zero-update steps).
      • Flow-fair scaling is capped (prevents rare-step explosion).
...
$$ g_t \leftarrow g_t + A\sin\Big(\frac{2\pi t}{T}\Big)v $$
...
Synthetic gate uses a combined summary:
                                                         ...
                                                         2
$$ \text{combined} = \frac{\text{adv(CVaR95)} + \text{adv(max)}}{2} $$
...
For each (\text{seed}, T, \text{base_schedule}):


     • identical initial weights (cloned init_state)
     • identical batch order (fixed-order loader)
     • identical forcing vectors (v_map)
     • identical RNG streams across schedules (CPU/NumPy/CUDA RNG snapshot restore)

5.2 Why seed-key intersection is final

All paired tests are performed using seed-key intersection:


     • build {seed → result} dictionaries
     • intersect seeds between schedule and baseline
     • test only on common seeds
...
Run the synthetic quadratic task using the same PSMC engine code path.
                                                      ...
                                                      3
Proceed to MNIST only if:


      • combined advantage mean ≥ 2% for at least one normalization mode.

7.2 Publishable positive

Claim schedule-specific benefit only if:


      • FDR-corrected p < 0.05 for at least one tail metric and
      • advantage ≥ 5% in ≥ 2 forcing periods (recommended robustness threshold).

Otherwise, the clean null is still valuable:


      • “No evidence primes outperform matched random under this forcing model.”
